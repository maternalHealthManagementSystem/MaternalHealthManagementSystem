...
export const useCalendarStore = defineStore('schedule', {
  ...
  actions: {
    ...
    async fetchAllData(user_id) {
      ...
      this.loading = true;
      try {
        const response = await api.get(`http://172.20.10.4:3001/api/schedule`,{
          headers: {
    ...
    async addEvent(newEvent) {
      try {
        ...
        await axios.post('http://172.20.10.4:3001/api/schedule', payload, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`
          }
        });
    ...
    async updateEvent(updatedEvent) {
      try {
        ...
        await axios.put(`http://172.20.10.4:3001/api/schedule/${updatedEvent.id}`, payload,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`
            }
    ...
    async deleteEvent(eventId) {
      try {
        await axios.delete(`http://172.20.10.4:3001/api/schedule/${eventId}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`
            }
    ...
    async addDiary(diary, imageFile) {
      try {
        ...
        await axios.post('http://172.20.10.4:3001/api/diary', formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`
            }
    ...
    async updateDiary(updatedDiary, imageFile) {
      try {
        ...
        await axios.put(`http://172.20.10.4:3001/api/diary/${updatedDiary.id}`, formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`
            }
    ...
    async deleteDiary(diaryId) {
      try {
        await axios.delete(`http://172.20.10.4:3001/api/diary/${diaryId}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`
            }
